...
pub async fn show(
    ...
    let person = guard
        .fetch_optional(sqlx::query_as::<_, Person>("SELECT * FROM persons WHERE id = $1").bind(id))
        .await;
...
pub async fn update(
    ...
    let result = guard
        .fetch_optional(sqlx::query_as::<_, Person>(
            "UPDATE persons
...
pub async fn destroy(
    ...
    let result = guard.execute(sqlx::query("DELETE FROM persons WHERE id = $1").bind(id)).await;
    let _ = guard.release().await;
