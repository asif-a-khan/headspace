...
pub async fn submit(
        ...
        .await
    {
        ...
        Ok(None) => {
            ...
            return (StatusCode::NOT_FOUND, Json(serde_json::json!({ "error": "Form not found." }))).into_response();
        }
        Err(e) => {
        ...
        if attr.is_required {
            ...
            if let Some(val) = values.get(code) {
                if val.is_null() || (val.is_string() && val.as_str().unwrap_or("").is_empty()) {
                    let label = attr.name.as_deref().or(attr.attribute_name.as_deref()).unwrap_or(code);
                    let _ = guard.release().await;
    ...
    let person_name = values.get("name")
        .or_else(|| values.get("person_name"))
        .and_then(|v| v.as_str())
        .unwrap_or("Unknown")
        .to_string();

    let person_email = values.get("email")
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            let _ = guard.release().await;
            return (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({ "error": "Failed to process submission." }))).into_response();
        }
    };
    ...
    if let Some(ref email) = person_email {
        let _ = guard.execute(
            sqlx::query("INSERT INTO person_emails (person_id, email, label) VALUES ($1, $2, 'work')")
                .bind(person_id).bind(email),
        ).await;
    }

    // Add phone if provided
    if let Some(phone) = values.get("phone").or_else(|| values.get("person_phone")).and_then(|v| v.as_str()) {
        if !phone.is_empty() {
            let _ = guard.execute(
                sqlx::query("INSERT INTO person_phones (person_id, phone, label) VALUES ($1, $2, 'work')")
                    .bind(person_id).bind(phone),
            ).await;
        }
    }
    ...
    if form.create_lead {
        ...
        let lead_title = values.get("title")
